"""
Picass plugin to perform BKM (Binding Kinetics Multiplexing) procedure on 
loaded data.
...
class MainBKMDialog(QtWidgets.QDialog):
	def __init__(self, window):
		super().__init__()
		self.window = window
		self.setWindowTitle("BKM")
	...
	def save_all_grids(self, names):
		...
		for i in range(len(self.barcodes_settings.barcodes_checks)):
			if self.barcodes_settings.barcodes_checks[i].isChecked():
				...
				new_info = {
					"Generated by": "Picasso Render BKM",
					"Pick Shape": "Circle",
...
class Plugin():
	...
	def execute(self):
		...
		BKM_action = pp_menu.addAction(
			"Perform BKM analysis"
		)
		BKM_action.triggered.connect(self.open_BKM_dialog)

		# add dialog
		self.window.bkm_dialog = MainBKMDialog(self.window)
		self.window.dialogs.append(self.window.bkm_dialog)

	def open_BKM_dialog(self):
