"""
Picass plugin to perform ML PAINT (Machine Learning PAINT) procedure on 
loaded data.
...
class MainMLDialog(QtWidgets.QDialog):
	def __init__(self, window):
		super().__init__()
		self.window = window
		self.setWindowTitle("MLPAINT analysis")
	...
	def save_all_grids(self, names):
		...
		for i in range(len(self.barcodes_settings.barcodes_checks)):
			if self.barcodes_settings.barcodes_checks[i].isChecked():
				...
				new_info = {
					"Generated by": "Picasso Render MLPAINT",
					"Pick Shape": "Circle",
...
class Plugin():
	...
	def execute(self):
		...
		ML_action = pp_menu.addAction(
			"Perform MLPAINT analysis"
		)
		ML_action.triggered.connect(self.open_ML_dialog)

		# add dialog
		self.window.ml_dialog = MainMLDialog(self.window)
		self.window.dialogs.append(self.window.ml_dialog)
